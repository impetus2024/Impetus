...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn(
        "sticky top-0 z-10 bg-muted/40 backdrop-blur supports-backdrop-filter:bg-muted/30 [&_tr]:border-b [&_tr]:border-border/70",
        className
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/60 transition-colors duration-150 hover:bg-muted/40 has-aria-expanded:bg-muted/50 data-[state=selected]:bg-muted",
        className
      )}
      {...props}
    />
